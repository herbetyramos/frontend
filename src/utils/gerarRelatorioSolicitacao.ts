...
export function gerarRelatorioSolicitacao(
    ...
    pdf.setFontSize(16);
    pdf.text("SOLICITAÇÃO DE MATERIAIS", largura / 2, 18, {
        align: "center",
    });
    ...
    pdf.text(dados.cronograma.tema || "-", 40, y);
    ...
    pdf.setFont("helvetica", "normal");
    pdf.text(dados.cronograma.professor?.nome || "-", 40, y);
    ...
    if (
        ...
        pdf.setFontSize(12);
        pdf.setFont("helvetica", "bold");
        pdf.text("MATERIAIS EXTRAS", 14, y);
        ...
        autoTable(pdf, {
            startY: y,
            head: [["Qtd", "Material"]],
            body: dados.materiaisExtras.map(item => [
                item.qtde,
                item.nome_material
            ]),
            styles: {
    ...
    pdf.line(20, 255, 90, 255);
    pdf.line(120, 255, 190, 255);

    pdf.setFontSize(9);

    pdf.text("Professor", 55, 260, {
        align: "center"
    });

    pdf.text("Almoxarifado", 155, 260, {
        align: "center"
    });

    pdf.save("SolicitacaoMateriais.pdf");
}