...
export default function Licitacao() {
  const [data_formatura, setData_formatura] = useState("");
  ...
  async function handleRegister(event: FormEvent) {
    ...
    try {
      ...
      await apiClient.post("/formatura", data);
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div className="relative w-full">
            <input
              type="text"
              id="Id_formatura"
              value={data_formatura}
              onChange={(e) => setData_formatura(e.target.value)}
              placeholder=" "
              required
